export class PathSelector {
  ...
  async toHTML(parent) {
    ...
    mainCont.className = 'path-selector-container';
    ...
    this.tower.upgrades.forEach((upgrade) => {
      ...
      for (let i = 0; i < pathNums.length; i++) {
        ...
        if (pathNums[i] < this.selected[i]) butt.className = 'path-selector-button-sub';
        else if (pathNums[i] == this.selected[i]) butt.className = 'path-selector-button-dom';
  ...
  getUpgradeIconSource(path) {
    ...
    const domPath = this.tower.getDominantPath(path);
    const upgrade = this.tower.upgrades.find(upgrade => upgrade.path == domPath);
    if (upgrade != null && upgrade.metadata != null && upgrade.metadata.iconSource != null) return upgrade.metadata.iconSource;



    return "/media/upgrade-icons/" + toFileName(this.tower.id) + "/" + domPath + ".png";
  }
